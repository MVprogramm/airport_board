...
const Button = ({ dayName, setFlyDay, dispatch }) => {
  const inputDayHandler = (event) => {
    switch (event.target.innerHTML) {
      case "Yesterday":
        dispatch(
          setFlyDay(getDate.getFormattedFilter(getDate.getYesterdaysDate())[0])
        );
        break;
      case "Today":
        dispatch(setFlyDay(getDate.getFormattedFilter(new Date())[0]));
        break;
      case "Tomorrow":
        dispatch(
          setFlyDay(getDate.getFormattedFilter(getDate.getTomorrowsDate())[0])
        );
        break;
    }
  };
  ...
  return (
    <div className="date__setter date__setter_btn" onClick={inputDayHandler}>
